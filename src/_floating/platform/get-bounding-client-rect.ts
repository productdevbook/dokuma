// 1:1 port of @floating-ui/dom/src/utils/getBoundingClientRect.

import { getComputedStyle, getFrameElement, getWindow } from "../dom.ts"
import type { DomVirtualElement } from "../types.ts"
import type { ClientRectObject } from "../utils.ts"
import { createCoords, rectToClientRect } from "../utils.ts"
import { isElement } from "./is-element.ts"
import { getScale } from "./get-scale.ts"
import { getVisualOffsets, shouldAddVisualOffsets } from "./get-visual-offsets.ts"
...
export function getBoundingClientRect(
  ...
  if (domElement) {
    ...
    while (currentIFrame && offsetParent && offsetWin !== currentWin) {
      ...
      const left =
        iframeRect.left +
        (currentIFrame.clientLeft + Number.parseFloat(css.paddingLeft)) * iframeScale.x
      const top =
        iframeRect.top +
        (currentIFrame.clientTop + Number.parseFloat(css.paddingTop)) * iframeScale.y
