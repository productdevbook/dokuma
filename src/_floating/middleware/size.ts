...
export const size = (options: SizeOptions | Derivable<SizeOptions> = {}): Middleware => ({
  ...
  async fn(state) {
    ...
    if (side === "top" || side === "bottom") {
      heightSide = side
      widthSide =
        alignment === ((await platform.isRTL?.(elements.floating)) ? "start" : "end")
          ? "left"
          : "right"
    } else {
      widthSide = side
